...
INLINE_CLIENT_ACCOUNT_STORE_DIR=${INLINE_CLIENT_ACCOUNT_STORE_DIR:-${DATA_DIR}/inline-client/accounts}
MATRIX_INLINE_DB_URI=${MATRIX_INLINE_DB_URI:-file:${DATA_DIR}/matrix-inline.db?_txlock=immediate}
DEFAULT_NETWORK_ICON=mxc://matrix.org/ITxccqHQkLCnPQDouWfsPhqs
...
MATRIX_INLINE_NETWORK_DISPLAYNAME=${MATRIX_INLINE_NETWORK_DISPLAYNAME:-}
MATRIX_INLINE_NETWORK_URL=${MATRIX_INLINE_NETWORK_URL:-}
MATRIX_INLINE_NETWORK_ICON=${MATRIX_INLINE_NETWORK_ICON:-${DEFAULT_NETWORK_ICON}}
MATRIX_INLINE_BOT_DISPLAYNAME=${MATRIX_INLINE_BOT_DISPLAYNAME:-}
MATRIX_INLINE_BOT_AVATAR=${MATRIX_INLINE_BOT_AVATAR:-}
PUID=${PUID:-1337}
PGID=${PGID:-1337}

export DATA_DIR CONFIG_PATH REGISTRATION_PATH INLINE_SIDECAR_BIND INLINE_SIDECAR_URL INLINE_CLIENT_STORE INLINE_CLIENT_ACCOUNT_STORE_DIR MATRIX_INLINE_DB_URI
...
export MATRIX_INLINE_NETWORK_DISPLAYNAME MATRIX_INLINE_NETWORK_URL MATRIX_INLINE_NETWORK_ICON MATRIX_INLINE_BOT_DISPLAYNAME MATRIX_INLINE_BOT_AVATAR

mkdir -p "${DATA_DIR}" "$(dirname "${INLINE_CLIENT_STORE}")" "${INLINE_CLIENT_ACCOUNT_STORE_DIR}"
...
function patch_profile_config {
	patch_config_value MATRIX_INLINE_NETWORK_DISPLAYNAME .network.displayname
	patch_config_value MATRIX_INLINE_NETWORK_URL .network.network_url
	patch_config_value MATRIX_INLINE_BOT_DISPLAYNAME .appservice.bot.displayname
	if [[ -n "${MATRIX_INLINE_NETWORK_ICON}" ]]; then
		CONFIG_VALUE="${MATRIX_INLINE_NETWORK_ICON}" yq -i '
			.network.network_icon = strenv(CONFIG_VALUE) |
			.appservice.bot.avatar = strenv(CONFIG_VALUE)
		' "${CONFIG_PATH}"
	fi
	patch_config_value MATRIX_INLINE_BOT_AVATAR .appservice.bot.avatar
}
...
su-exec "${PUID}:${PGID}" "${SIDECAR_NAME}" \
	...
	--account-store-dir "${INLINE_CLIENT_ACCOUNT_STORE_DIR}" \
